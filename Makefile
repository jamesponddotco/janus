...
doc: # Builds the manpage.
	$(SCDOC) <doc/janus.1.scd >doc/janus.1

install: # Installs the release binary.
	$(INSTALL) -d \
		$(DESTDIR)$(PREFIX)/$(BINDIR)/ \
		$(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
	$(INSTALL) -pm 0755 janus $(DESTDIR)$(PREFIX)/$(BINDIR)/
	$(INSTALL) -pm 0644 doc/janus.1 $(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
